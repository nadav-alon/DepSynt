...
def get_environment_transitions(rows, cols):
    ...
    for i, col in enumerate(cols):
        if i == 0: continue
            ...
            trans_list.append(f"({col} -> X(l -> {cols[i-1]}))")
        else:
            trans_list.append(f"({col} -> X(l -> ({cols[i-1]} | {cols[i-2]})))")
    # action right
    for i, col in enumerate(cols):
        if i == len(cols)-1: continue
        elif i == len(cols)-2:
            trans_list.append(f"({col} -> X(r -> {cols[i+1]}))")
        else:
            trans_list.append(f"({col} -> X(r -> ({cols[i+1]} | {cols[i+2]})))")
    # action up
    for i, row in enumerate(rows):
        if i == 0: continue
        elif i == 1:
            trans_list.append(f"({row} -> X(u -> {rows[i-1]}))")
        else:
            trans_list.append(f"({row} -> X(u -> ({rows[i-1]} | {rows[i-2]})))")
    # action down
    for i, row in enumerate(rows):
        if i == len(rows)-1: continue
        elif i == len(rows)-2:
            trans_list.append(f"({row} -> X(d -> {rows[i+1]}))")
        else:
            trans_list.append(f"({row} -> X(d -> ({rows[i+1]} | {rows[i+2]})))")
    return trans_list

def get_same_next_state(rows, cols):
    next_list = []
    for col in cols:
        next_list.append(f"({col} -> X(u -> {col}))")
        next_list.append(f"({col} -> X(d -> {col}))")
    for row in rows:
        next_list.append(f"({row} -> X(l -> {row}))")
        next_list.append(f"({row} -> X(r -> {row}))")
    return next_list

def generate_ltlf(gridsize, init_pos=(1,1), goal_pos=(4,4)):
    ...
    env_str = f"((({mut_exc_env_rows_all}) & {mut_exc_env_rows_comb}) & (({mut_exc_env_cols_all}) & {mut_exc_env_cols_comb}) & ({' & '.join(env_transitions)}))"
    ...
    return combined, rows + cols, agent_actions

def generate_tlsf(gridsize, init_pos=(1,1), goal_pos=(4,4)):
    rows, cols = generate_locations(gridsize)
    agent_actions = ["l", "r", "u", "d"]
...
"""
    ...
    tlsf = template.format(
        ...
        env_trans_f=to_tlsf_expr(" && ".join(env_transitions)),
...
def main():
    ...
    parser.add_argument("--goal", type=int, nargs=2, default=[4, 4], help="Goal position (row col)")
    parser.add_argument("--output", type=str, help="Output file path")
    args = parser.parse_args()
    
    if args.format == "ltlf":
        content, inputs, outputs = generate_ltlf(args.gridsize, tuple(args.init), tuple(args.goal))
        prefix = f".inputs: {' '.join(inputs)}\n.outputs: {' '.join(outputs)}\n"
        content = prefix + content
    elif args.format == "tlsf":
        content = generate_tlsf(args.gridsize, tuple(args.init), tuple(args.goal))
    else: # both
        ltlf, inputs, outputs = generate_ltlf(args.gridsize, tuple(args.init), tuple(args.goal))
        tlsf = generate_tlsf(args.gridsize, tuple(args.init), tuple(args.goal))
        content = f"--- LTLf ---\n.inputs: {' '.join(inputs)}\n.outputs: {' '.join(outputs)}\n{ltlf}\n\n--- TLSF ---\n{tlsf}"
